...
impl FromWif for PublicKey {
    ...
    fn from_wif(wif: impl AsRef<[u8]>) -> Result<Self, Self::Err> {
        ...
        let decoded = bs58::decode(&wif[3..])
            .into_vec()
            .map_err(PublicKeyWifError::Decode)?;
        ...
        Ok(Self {
            key: secp256k1::PublicKey::from_slice(key).map_err(PublicKeyWifError::Secp256k1)?,
            prefix,
        })
    }
}
    ...
    fn wif_to_compressed_public_key() {
        ...
        assert_eq!(
            ...
            secp256k1::PublicKey::from_str(
                "030259b4961bed2db07dfa61983d538b9dd11e66777f2931558f3267c613485ec6"
            )
            .unwrap()
        );
